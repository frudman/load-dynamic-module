...
const isCommonJS = code => /module[.]exports/.test(code); // convert a dependancy reference to an http-gettable url
...
class Module {
  ...
  resolvedWithError(err) {
    this.err = err; // now, test 'err.message' to give friendly hint: e.g. from chrome: 'await is only valid in async function'
...
async function loadModule(moduleRequestUrl, {
  ...
  return new Promise(async resolve => {
    ...
    const module = loadedModules[actualModuleUrl] || (loadedModules[actualModuleUrl] = new Module({
      name: actualModuleUrl
    }));
