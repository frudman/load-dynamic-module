...
function createLoader(baseConfig, overrides = {}) {
  ...
  customLoader.config = createLoader.bind(null, customConfig); //customLoader.all = () => loadedModules; // when debugging
...
function genModuleInitMethods(preloadSubModules, getPreloadedModule, cjs) {
  ...
  const getExports = m => m instanceof RequiredModuleMissingError ? m : exportsUsed() ? exports : moduleExportsAssigned() ? module.exports : m; // IMPORTANT: all UMD modules test for 'define.amd' being 'truthy'
  ...
  }; //cjs?{ require, module, exports }:{ define, require }};
}
