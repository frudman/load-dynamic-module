...
async function loadModule(moduleRequestUrl, {
  ...
  return new Promise(async resolve => {
    ...
    if (module.isLoaded) {
      ...
      try {
        const m = await (0, _tidbits.http)(actualModuleUrl);
        log('noty', m);
        ...
        const initModule = new _tidbits.AsyncFunction(...Object.keys(amdProxy), code); // IMPORTANT: all AMD modules test for 'define.amd' being 'truthy'
...
(async function () {
  ...
  try {
    const x = await (0, _tidbits.http)(lib(linName));
    const data = JSON.parse(x.data); // .results = [], .total = number
    // 

    logr(data);
    const y = await (0, _tidbits.http)(search(linName));
