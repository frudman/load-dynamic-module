...
///index.mjs';///utils.js';///xutils.js';//'tidbits';//'./http-get'; // instead of axios.get (lighter code base)
...
async function loadModule(moduleRequestUrl, {
  ...
  return new Promise(async resolve => {
    ...
    if (module.isLoaded) {
      ...
      try {
        const m = await (0, _freddyJavascriptUtils.http)(actualModuleUrl);
        log('noty', m);
        ...
        if (/text[/]css/.test(m.dataType)) {
          ...
          log('WHHOPS, not javascript code???', m.data); // resolve with error? if plain text, try to exec?
        ...
        const initModule = new _freddyJavascriptUtils.AsyncFunction(...Object.keys(amdProxy), code); // IMPORTANT: all AMD modules test for 'define.amd' being 'truthy'
...
(async function () {
  ...
  const cdnjs = what => `https://api.cdnjs.com/${what}`,
        search = name => cdnjs(`libraries?search=${name}`),
        // .results = [], .total = number; each result: { .name, .latest: file-url-of-latest-version (presumably minifiled)}
  lib = name => cdnjs(`libraries/${name}`),
        // .name, .filename, .assets = [{ .version, .files=[ 'core.js', 'jquery.js', 'jquery.min.js',... ]}...]
  libv = name => cdnjs(`libraries/${name}?fields=name,filename,version`);

  try {
    const x = await (0, _freddyJavascriptUtils.http)(lib(linName));
    const data = JSON.parse(x.data); // .results = [], .total = number
    // 

    logr(data);
    const y = await (0, _freddyJavascriptUtils.http)(search(linName));
