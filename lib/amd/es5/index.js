    ...
    for (const dep of args) {
      if (typeof dep === 'string') {
        ...
        if (url) {
          ...
          if (depModule.isLoaded) downloads.push(depModule.module); // no need for a promise, already resolved
              ...
              downloads.push(new Promise(resolvex => {
                ...
                const done = module => {
                  depModule.resolved(module);
                  resolvex(depModule.module);
                }; // depModule.module === module
...
async function initJSModule(config, moduleUrl, moduleSourceCode) {
  ...
  return new Promise(async resolveJSM => {
    ...
    const dependenciesLoader = privateLoader.bind(subModulesConfig); // basic safety & better performance: is that safe for every module?
    ...
    const {
      ...
      moduleGlobals,
      getExports
    } = genModuleInitMethods(subModulesConfig, resolveJSM, dependenciesLoader);
...
function genModuleInitMethods(config, moduleResolve, loadSubModules) {
  ...
  const getExports = () => exportsUsed() ? exports : moduleExportsAssigned() ? module.exports : undefined; // IMPORTANT: all UMD modules test for 'define.amd' being 'truthy'
  ...
  async function define(...args) {
    ...
    if (args.length === 0) {
      ...
      if (moduleDefine.length === 0) {// ...and not using require/exports/module form...
        // so we're all good
      } else {
      ...
      if (Array.isArray(depsArray)) {
        depsArray.length > 0 && externalDeps.push(...(await loadSubModules(...depsArray))); // always an array (as per config.alwaysAsArray)
      } else return resolveModuleAs(new ModuleLoadError(`expecting '[dependencies]' to be an array (was ${typeof externals})`));
    }
