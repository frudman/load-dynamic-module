...
async function initJSModule(config, moduleUrl, moduleSourceCode) {
  ...
  return new Promise(async resolveJSM => {
    ...
    const {
      ...
      moduleGlobals,
      getExports
    } = genModuleInitMethods(dependenciesLoader, preloadedModules, resolveJSM);
  ...
  async function define(...args) {
    ...
    if (args.length === 0) {
      ...
      if (Array.isArray(depsArray)) {
        depsArray.length > 0 && externalDeps.push(...(await preloadSubModules(...depsArray))); // always an array (as per config.alwaysAsArray)
      } else return resolveModuleAs(new ModuleLoadError(`expecting array of dependencies (was ${typeof externals})`));
    }
