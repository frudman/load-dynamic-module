...
class Module {
  ...
  resolved(m) {
    ...
    this.waitingOnMe.forEach(resolveDep => resolveDep()); //delete this.waitingOnMe; // why not...
      ...
      for (const dep of resolvedDeps) {
        if (!('finalVALUE' in dep)) {
          ...
          try {
            ...
            } else dep.finalVALUE = initialVALUE; // now final forever
        ...
        if (module.isLoaded) {
          ...
          try {
            ...
            try {
              ...
              log('GOT AMD RESPONSE for', moduleUrl, zz); //module.resolved(zz);
            } catch (err) {
            ...
            if (!AMD_MODULE.isAMD) {
              if (isCommonJS(moduleSourcecode)) {
                log('TRYING for CJS CODE', moduleUrl); // pass #2: yes, less efficient (since 2 passes) but allows for both modes (i.e. amd/umd and cjs) to be imported
