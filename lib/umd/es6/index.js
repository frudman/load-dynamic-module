...
async function privateLoader(config, ...args) {
  ...
  return new Promise(resolveWhenReady => {
    ...
    Promise.all(downloads).then(async resolvedDeps => {
      ...
      try {
        const mods = resolvedDeps.map(dep => dep.finalVALUE); // if (onReady)
        //     resolveWhenReady(onReady(...mods)); // result is whatever the onReady returns...
        // else 

        resolveWhenReady(resolvedDeps.length === 1 ? mods[0] : mods); //     resolveWhenReady(mods[0]); // could be: undefined, module, or error
        // else
