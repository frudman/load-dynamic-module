...
function awaitGood(code) {
    ...
    // can detect if not valid with AsyncFunction: err.name === 'SyntaxError'; err.message === 'SyntaxError: await is only valid in async function'
...
async function loadModuleByUrl(moduleRequestUrl, askingUrl = window.location.href) {
