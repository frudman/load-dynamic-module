...
const getModule = id => loadedModules[id] || (loadedModules[id] = new DynamicModule({
  id
}));
...
class DynamicModule {
  ...
  genAMDDefine(subDepResolution) {
    ...
    function defineMethod(...args) {
      ...
      if (typeof moduleDefine !== 'function') throw new ModuleLoadError(`expecting 'define' to be a function (was ${typeof moduleDefine})`); // SIMPLE STRATEGY 1: implement as AMD wants

      const externals = args.pop() || [];
      if (!Array.isArray(externals)) throw new ModuleLoadError(`expecting '[dependencies]' to be an array (was ${typeof externals})`);
...
function addKnownModule(ref, module, custom = []) {
  ...
  addModule(name, new DynamicModule({
    id,
    module
  }));
}
  ...
  return new Promise(resolveWhenReady => {
    ...
    async function initJSModule(moduleUrl, moduleSourceCode) {
      ...
      return new Promise(async resolveJSM => {
        ...
        useStrict && (moduleSourceCode = '"use strict";\n\n' + moduleSourceCode); // get existing module (if already loaded), or creates new one
        ...
        if (module.isLoaded) {
          ...
          const amdProxy = {
            ...
            require(ref) {
              throw new ModuleLoadError(`cannot 'require' in AMD ${modID}:\n\ttry 'await requireAsync("${ref}"' instead`);
            },
          ...
          const cjsProxy = {
            ...
            define: () => {
              throw new ModuleLoadError(`unexpected use of DEFINE in commonJS ${modID}`);
            },
            module: {
              exports: cjsExports
            },
          ...
          try {
            ...
            const nonAMDResult = await initModule(...Object.values(amdProxy)); // no errors
            // if amd, we're done
            ...
            if (AMD_MODULE.isAMD) {// we're done: module was already resolved through define method
            } else {
            ...
            if (AMD_MODULE.isAMD) module.resolved(new ModuleLoadError(`AMD ${modID} error`, err)); // consider it resolved (albeit with errors)
