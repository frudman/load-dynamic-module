...
var _freddyJavascriptUtils = require("my-npm-packages/freddy-javascript-utils");
...
const loadedModules = {}; // cheap means to ensure no infinite loop while resolving dependencies
...
class Module {
  ...
  static getModule(name) {
    ...
    return loadedModules[name] || (loadedModules[name] = new Module({
      name
    }));
  }
  ...
  dependsOnMe(resolveDependent) {
    ...
    if (deps.length > MAX_LIKELY_NUMBER_OF_DEPENDENTS) this.resolvedWithError(new Error(`likely cycle in module resolution for ${this.name} (depth=${deps.length})`));
  }
  ...
  genAMDDefine(subDepResolution) {
    ...
    const thisModule = this; // keep track of whether or not our define method was actually called...
  ...
  return new Promise(resolveWhenReady => {
    ...
    for (const dep of args) {
      if (typeof dep === 'string') {
        ...
        globalName = m && m[1] && m[1].slice(0, -1),
        ...
        if (url) {
          // DOWNLOAD DATA
          const finalUrl = urlResolver(url, baseUrl); // see if a module and if already there
          ...
          if (inProgress) {
            ...
            downloads.push(alreadyInProgress[finalUrl] = (0, _freddyJavascriptUtils.http)(finalUrl).then(downloaded => ({
        ...
        if (module.isLoaded) {
          ...
          try {
            ...
            const cjsProxy = {
              ...
              require: async nameOrUrl => (await privateLoader(subDepResolution, nameOrUrl)).module // customize proxies & globals as needed
            ...
            const initModule = new _freddyJavascriptUtils.AsyncFunction(...Object.keys(amdProxy), moduleSourcecode);
            ...
            try {
              ...
              AMD_MODULE.isAMD && module.resolvedWithError(err); // else, fall through and see if it works with CJS below
            }
            ...
            if (!AMD_MODULE.isAMD) {
              if (isCommonJS(moduleSourcecode)) {
                ...
                const commonjsInit = new _freddyJavascriptUtils.AsyncFunction(...Object.keys(cjsProxy), awaitableCode);
